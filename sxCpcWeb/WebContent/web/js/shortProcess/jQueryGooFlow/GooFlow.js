...
//初始化区域图的对象
	...
	if(this.$editable){
	  this.$workArea.on("click",{inthis:this},function(e){
		...
		This.addNode(This.$id+"_node_"+This.$max,{name:"任务名",left:X,top:Y,type:This.$nowType});
		This.$max++;
	  });
	  //划线或改线时用的绑定
	  ...
	  this.$textArea=$("<textarea placeholder=''></textarea>");
...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		...
		$(this.$draw).delegate(tmpClk,"dblclick",{inthis:this},function(e){
			$("textarea").attr("placeholder","1:串行 2:并行 3:无效");
			//add by dangzw begin 2016-11-02
			var flag=$(this).attr("flag");
			//1 代表详细页面的标识 双击线时 不可编辑
			if(flag=="1" || flag=="2"){
				return false;
			}
			//add by dangzw begin 2016-11-02
			...
			This.$workArea.parent().one("mousedown",function(e){
				//add by dangzw begin 2016-11-30
				var lineName=This.$textArea.val();
				var reg=/^[1|2|3]$/;
				if(lineName!=""){
					if(!reg.test(lineName)){
						layer.alert("请填写数字1或者2,1代表串行,2代表并行,3代表无效!",8);
						This.$textArea.val("");
						$(".GooFlow_line_oper").hide();
					}
				}
				//add by dangzw end 2016-11-30
				if(e.button==2)return false;
	...
	addNode:function(id,json){
		...
		if(json.type.indexOf(" round")<0){
			...
			this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'><i class='ico_"+json.type+"'></i></td><td id='td_"+id+"'>"+json.name+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
		}
		else{
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".GooFlow_item","click",{inthis:this},function(e){
			...
			//add by dangzw end 2016-11-01
			
			//add by dangzw begin 2016-11-29
			//workflowCustomType:0 省定制	 1 省市协同定制
			if(nodeType=="0" || nodeType=="2"){
				return false;
			}
			if(flag=="2" && regionId=="888" && workflowCustomType=="1"){
				layer.alert("该流程定制类型属于【省市协同定制】,请在市级工号人员下维护节点信息!",8);
				return false;
			}
			if(flag=="2" && regionId!="888" && workflowCustomType=="0"){
				layer.alert("该流程定制类型属于【省定制】,请在省级工号人员下维护节点信息!",8);
				return false;
			}
			//add by dangzw end 2016-11-29
			e.data.inthis.focusItem(this.id,true);
		...
		this.$workArea.delegate(".ico","mousedown",{inthis:this},function(e){
			//add by dangzw begin 2016-11-01
			var flag=$(this).parents(".GooFlow_item").attr("flag");
			if(flag=="1" || flag=="2"){
				return false;
			}
			//add by dangzw end 2016-11-01
		...
		this.$workArea.delegate(".GooFlow_item > .span","dblclick",{inthis:this},function(e){
			...
			This.$workArea.parent().one("mousedown",function(e){
				//add by dangzw begin 2-16-11-30
				var nodeValue=This.$textArea.val();
				if(nodeValue==""){
					layer.alert("节点名称不能为空!",8);
					This.$textArea.val("任务名");
				}
				//add by dangzw end 2-16-11-30
				if(e.button==2)return false;
