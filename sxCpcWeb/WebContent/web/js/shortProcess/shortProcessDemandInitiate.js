var ShortProcessDemandInitiate = new Function();
ShortProcessDemandInitiate.prototype = {
	...
	bindMetod : function(parentThis) {
				...
				if(json.code == "0" ){
					...
					$.each(dataList,function(i,obj){
						...
						if(obj.ATTR_TYPE=="radio"){
							...
							html.push('<input id='+obj.ATTR_ONAME+' tempId='+obj.ATTR_ONAME+' type='+obj.ATTR_TYPE+' name="radios" style="width: 2%;">'+obj.ATTR_NAME+'');
		...
		//add by dangzw end 2016-12-07
		//生成需求ID
		var param={};
		param.handleType="qryLst";
		param.dataSource="";
		param.nameSpace="shortProcess";
		param.sqlName="queryWorkFlowNeedId";
		$.jump.ajax(URL_QUERY_COMMON_METHOD.encodeUrl(), function(json) {
			if(json.code == "0" ){
...
//			 inputElement.addEventListener("change", handleFiles, false); 
			...
			var demand_id =parentThis.demandId;
			var upload_person_name=$("#releasePersonName").val();
			//查询文件保存的地址
			var file_path="";
			var param1={
			...
			var option = { 
        			...
        			success: function (json) {
        					...
        					$.jump.ajax(URL_QUERY_GOVER_ENTER.encodeUrl(), function(json) {
 							...
 							html.push('<div   id="divObj'+i+'" class="lable-title fl" style="width:15%;"><a href="javascript:void(0)" name="attachment" attachment_name="'+obj.attachment_name+'" attachment_path="'+obj.attachment_path+'" other_attachment_name="'+other_attachment_name+'" style="color: #4782DD; text-decoration: underline;width:200px;" title="上传人：'+upload_person_name+',文件名：'+obj.attachment_name+'">'+attachment_name+'</a><a  id="'+i+'" href="javascript:void(0)" name="attachmenta" attachment_name="'+obj.attachment_name+'" attachment_path="'+obj.attachment_path+'" other_attachment_name="'+other_attachment_name+'" style="color: red;width:200px;">×</a></div>');
            				...
            				$("#demandDetailInfo").find("a[name=attachmenta]").unbind("click").bind("click",function(){
            					...
            					var parama={								
            							...
            							"attachment_value" : parentThis.demandId,
		...
		/* 保存草稿
		var backObj=parentThis.selecter.findById("a","saveInfo")[0];
		backObj.unbind("click").bind("click",function(){
			parentThis.saveInfo(parentThis);
		});*/
		// 详细页面加载数据
			 	...
			 	$.jump.ajax(URL_QUERY_WORKFLOW.encodeUrl(), function(json) {
			  	...
			  	//判断下一节点的处理人及处理部门是否存在
				 	if(parentThis.node_executor_id==""){
				 		//modify 2017-10-23 如果dept_level层级是4，说明流程配置的时候是按照发起人部门领导来配置的，此规则时，需要重新查询应该按照哪个层级来做
				 		if(dept_level=="4"){
				 			var paramam={								
									"handleType"  		: 	"qryLst",
									"dataSource"  		: 	"",
...
/** ******************************************加载流程图end************************************************** */		
	...
	submitInfo : function(parentThis){
		...
		var  workflowName =parentThis.demandInfo.workflowName;  //流程ID
		 ...
		 $.jump.ajax(URL_ADD_WORKFLOWNEED.encodeUrl(), function(json) {
				if(json.code == "0" ){
					layer.alert("发起成功",1);
					$.jump.loadHtmlForFun("/web/html/shortProcess/flowList.html".encodeUrl(),function(pageHtml){
										$("#content").html(pageHtml);
