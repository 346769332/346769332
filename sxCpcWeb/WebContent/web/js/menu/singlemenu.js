...
TopMenu1.prototype = {
	...
	init : function(contentDiv) {
		...
		$.jump.ajax(URL_READ_JURISDICTION.encodeUrl(), function(data) {
			...
			if (data.code == "0") {
				//console.log("ww_________________:"+data.menuLst);
				//console.log("ww_________________:"+data.homePageLst[0].fun_path+"____"+data.homePageLst[0].fun_url);
				if (data.menuLst != null && data.menuLst.length > 0) {
					//temp.bindMetod(temp, data);
					
					//首页先写死到单池
//					if(data.homePageLst != null && data.homePageLst.length > 0) {
						parentThis.loadHtml("/web/html/top/indexsingle.html",parentThis, "indexsingle","");
//					}else {
