...
PoolList.prototype = {
	...
	queryPageList : function(parentThis,pageIndex) {
		...
		common.pageControl.start(URL_QUERY_POOL_LIST.encodeUrl(),
								 pageIndex,
								 parentThis.pageSize,
								 param,
								 "data",
	...
	createLstHtml : function(parentThis,data,listBodyObj){
		...
		if(dataLst.length > 0 ){
			...
			$.each(data.data,function(i,obj){
				html.push('<tr name="poolInfo" poolId="'+obj.POOL_ID+'" poolName="'+obj.POOL_NAME
						+'" loginCode="'+obj.LOGIN_CODE+'" loginName="'+obj.LOGIN_NAME+'" latnId="'+obj.REGION_ID+'" mobTel="'+obj.DEFAULT_MOB_TEL+'" orgId="'+obj.ORG_ID+'">');
				html.push('<td ><input id="poolInfoBox_'+obj.POOL_ID+'" name="poolInfoBox" type="checkbox" ></td>');
				//html.push('<td >'+obj.POOL_ID+'</td>');
				html.push('<td >'+obj.POOL_NAME+'</td>');
				html.push('<td >'+obj.CALIM_LIMIT+'</td>');
				html.push('<td >'+obj.URGE_SPAN_TIME+'</td>');
//				html.push('<td >'+obj.DEFAULT_EVAL_TIME+'</td>');
//				html.push('<td >'+obj.DEFAULT_EVAL+'</td>');
				html.push('<td >'+obj.LOGIN_NAME+'</td>');
				html.push('<td >'+obj.DEFAULT_MOB_TEL+'</td>');
	...
	updateStaffInfo:function(parentThis,poolInfo){
		...
		poolInfoPageDiv.find("a[name=infoSubmit]").unbind("click").bind("click",function(){
			...
			var confirm=layer.confirm('您是否确认提交？', function(){
				...
				$.jump.ajax(URL_QUERY_POOL_LIST.encodeUrl(), function(json) {
					if(json.code == "0" ){
						layer.close(confirm);
						layer.close(poolInfoPage);
						parentThis.queryPageList(parentThis,0);
