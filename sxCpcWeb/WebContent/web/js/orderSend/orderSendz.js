var OrderSendz = new Function() ;
OrderSendz.prototype = {
		...
		bindMetod : function(parentThis){
			...
			$.jump.ajax(URL_SYSUSERINFO_QUERY.encodeUrl(), function(data) {
				sendUserNameObj.val(data.systemUser.staffName);
	 			sendUserNameObj.attr("staffId",data.systemUser.staffId);
	 			sendUserNameObj.attr("orgId",data.systemUser.orgId);
	 			sendUserNameObj.attr("orgName",data.systemUser.orgName);

	 			$("#tel").val(data.systemUser.mobTel);
			 	orgNameObj.text(data.systemUser.orgName);
			}, param, false,false);
			...
			//附件上传与展示
//			 $("#attachSub").unbind("click").bind("click",function(){
//					
//					if ($("#attachment").val() == "") { 
//						layer.alert("请选择一个图片文件，再点击上传。",8); 
//						return; 
//						} 
...
//		            			 $.jump.ajax(URL_QRY_ATTACH_INFO.encodeUrl(),function(data) {	
//		            				 common.loadding("close");
//		            				
//		            				 $("#attachment").val("");
//		            			
//			            				 
//			            					if(data.code == "0") {
//			            					var attachInfoLst = data.attachInfoLst;	            					
//			             					var html=[];
//			             					if(attachInfoLst != null && attachInfoLst.length>0) {
//			             						
//			             						$.each(attachInfoLst,function(i,obj){	             					
//			             					 		base=data.encoder;
//			             					 		var fileString="data:image/jpeg;base64,"+base;	             						
//			             					 		html.push('<div name="photoGird" class="grid item" imgSrc="'+fileString+'" ><img src="'+fileString+'"/></div>');
//			             							$("#orderSendPagez").find("#attachmentInfo").html(html.join(''));
//			             							$("#orderSendPagez").find("div[name=photoGird]").unbind("dblclick").bind("dblclick",function(){
//			             				 				
...
//				});			
			...
			$("#attachSubs").unbind("click").bind("click",function(){
				...
				//上传文件  "demand_id="+parentThis.demandId+"&type=demand
				var param={
						"demand_id" :demand_id,
						"type"  : "demand"
				};
				...
				var option = { 
	        			...
	        			success: function (json) { 
            					...
            					var attachment_name = obj.attachment_name.split('.')[0];
     							...
     							html.push('<div  id="divObj'+i+'" class="lable-title fl" style="width:220px;"><a href="javascript:void(0)" name="attachment" attachment_name="'+obj.attachment_name+'" other_attachment_name="'+other_attachment_name+'" attachment_path="'+obj.attachment_path+'" style="color: #4782DD; text-decoration: underline;width:200px;" title="'+obj.attachment_name+'">'+attachment_name+'</a><a  id="'+i+'" href="javascript:void(0)" name="attachmenta" attachment_name="'+obj.attachment_name+'" attachment_path="'+obj.attachment_path+'"style="color: red;width:200px;">×</a></div>');
     							
             					$("#attachmentInfoY").append(html.join());
	             				
	            				//下载
	            				$("#orderSendPagez").find("a[name=attachment]").unbind("click").bind("click",function(){
	            				...
	            				//删除
	            				$("#orderSendPagez").find("a[name=attachmenta]").unbind("click").bind("click",function(){
	            					//获取删除按钮的ID
	            					var id=$(this).attr("id");
...
//								"attachment"		:		 base,	
							...
							$.jump.ajax(URL_SAVE_DEMAND_INFO.encodeUrl(), function(data) {
								if(data.code == "0"){
									layer.alert("需求单提交成功",9);
										$.jump.loadHtmlForFun("/web/html/orderSend/orderSendz.html".encodeUrl(),function(menuHtml){
											$("#content").html(menuHtml);
											var orderSendz = new OrderSendz();											
											var statusCd='{"regionCode":'+parentThis.regionCode+'}';											
											orderSendz.init(statusCd);
										});
								}else{
									var demandIdObj = parentThis.selecter.findById("td","demandId")[0];
	...
	var saveObj = parentThis.selecter.findById("a","save")[0]; 
			
			if(flag == "success"){
				saveObj.unbind("click").bind("click",function(){
					var flag = parentThis.checkData();
...
//								"attachment"		:		 base,	
		...
		//提交数据校验
		checkData : function(){
			  if("290"==this.regionCode  || true){//西安市的必须有接单部门
