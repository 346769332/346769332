...
var common = {
		...
		go : {
			next : function(path){
				...
				common.cachePath.addUrl(newURl);
		...
		cachePath:{
			addUrl : function(Url){
				var cachePathUrls = sessionStorage.getItem("cachePathUrls");
				var urlPath = {};
				if(cachePathUrls && cachePathUrls.length>0){
					urlPath = eval("("+cachePathUrls+")");
				}else{
					urlPath["urlPath"] = [];
				}
				
				urlPath["urlPath"].push(Url);
				sessionStorage.setItem("cachePathUrls",JSON.stringify(urlPath));
			},
			goBackUrl : function(){
				
				var cachePathUrls = sessionStorage.getItem("cachePathUrls");
				var urlPath = eval("("+cachePathUrls+")");
				var urlSet = urlPath["urlPath"];
				alert(urlSet);
				urlSet.remove(urlSet.length-1);
				
				sessionStorage.setItem("cachePathUrls",JSON.stringify(urlPath));
				
				var backUrl = urlSet[urlSet.length-1];
				
				window.location.href = "./"+ backUrl;
			},
			emptyCache : function(){
				sessionStorage.removeItem("cachePathUrls");
			}
		},
