...
@WebServlet("/shortProcess/cancleWorkflownode.do")
public class CancleWorkflowNodeServlet extends AbstractServlet{
	...
	public CancleWorkflowNodeServlet() {
		super();
	...
	protected void doPost(HttpServletRequest request,
		...
		String flag=request.getParameter("flag");//0,处理过程中的作废，1，发起人申请作废
		//处理时限
		int timeLimit=0;
		if(!flag.equals("1")){
			timeLimit=Integer.parseInt(String.valueOf(request.getParameter("timeLimit")));
		}
		//是否超时
		...
		String start_staff_id=request.getParameter("start_staff_id");
		String start_staff_name=request.getParameter("start_staff_name");
		String reason_info=request.getParameter("reason_info");
		...
		//reqMap.put("sendStaffName", request.getParameter("sendStaffName"));//发起人名
		//reqMap.put("sendStaffTel", request.getParameter("sendStaffTel"));//发起人电话
		if(flag.equals("0")){
		...
		reqMap.put("start_staff_id", start_staff_id);
		reqMap.put("start_staff_name", start_staff_name);
		reqMap.put("reason_info", reason_info);
		reqMap.put("mob_tel", mob_tel);
		...
		reqMap.put("SERVER_NAME", "cancleWorkflowNoed");
