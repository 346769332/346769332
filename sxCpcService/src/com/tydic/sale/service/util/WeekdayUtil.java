...
//	        	}
	 ...
	 */
	private static void setWorkLength(String time){
		if(null==time || "".equals(time) || time.length()!=19 ){
			time=yMdHms.format(new Date());
		}
		inputTime=time;
	...
	private static void setDateConfigData(String latn_id){
		...
		Map<String, Object> rltMap= searchService.getDateConfigData(reqMap);
		if("0".equals(rltMap.get("code"))){
			...
			Map  timeData=(Map) rltMap.get("timeData");
